(function (window, undefined) {
    var MapsLib = function (options) {
        ...
        this.locationColumn = options.locationColumn || "'attributes/field_address'";
        
        // appends to all address searches if not present
        this.locationScope = options.locationScope || "Ontario";
        ...
        $('.searchButton').bind('click', newSearch);

        function checkAll() {
          $("input[name='program-filter']").each(function(){
        ...
        if (address !== "") {
            ...
            self.geocoder.geocode({
                ...
                if (status === google.maps.GeocoderStatus.OK) {
                    ...
                    searchLat = results[0].geometry.viewport.f.b;
                    searchLng = results[0].geometry.viewport.b.f;
    ...
    MapsLib.prototype.doSearch = function (searchLat, searchLng) {
        var self = this;
        self.clearSearch();
        ...
        var type_column = "'Audience'";
        var tempWhereClause = [];

        $("input[name='program-filter']").each(function(){
            if(this.checked){
                if($(this).attr('value') != ""){
                    tempWhereClause.push($(this).attr('value'));
                }
            }
        });
    ...
    var self = this;
    var selectColumns = "'Audience', 'Email', 'attributes/title', 'attributes/field_accessibility', 'attributes/field_address', 'attributes/field_hours', 'attributes/field_orgname', 'attributes/field_latitude', 'attributes/field_longitude'";
  ...
  MapsLib.prototype.displayList = function(json) {
    ...
    var ontarioLat = 51.2538;
    var ontarioLng = -85.3232;

    var data = json['rows'];

    distanceSet(data, ontarioLat, ontarioLng);
...
function distanceSet(data, ontLat, ontLng){

    var searchAddress = document.getElementById("search_address").value;
    var distance = 0;  

    for (var pin in data){
        if(searchAddress){
            console.log("yes search Address");
            distance = Math.abs(getDistance(searchLat, searchLng, data[pin][7], data[pin][8]));
        }else{
            console.log("no search address");
            distance = getDistance(ontLat, ontLng, data[pin][7], data[pin][8]);
        }
